...
class BooksUrlsTest(SimpleTestCase):
    """Набір тестів для перевірки коректності маршрутизації API та сторінок (urls.py) застосунку books."""
    ...
    def test_perf_demo_url_resolves(self) -> None:
        """НОВИЙ ТЕСТ: Перевірка, що URL 'api/perf-demo/' успішно веде на ORMPerformanceListView."""
        url = reverse('books:perf_demo')
        match = resolve(url)

        self.assertEqual(match.view_name, 'books:perf_demo')


def test_cached_catalog_url_resolves(self) -> None:
        """Перевірка, що URL каталогу кешування веде на правильний клас CachedBooksListView."""
        url = reverse('books:cached_books_catalog')
        match = resolve(url)
        self.assertEqual(match.view_name, 'books:cached_books_catalog')
